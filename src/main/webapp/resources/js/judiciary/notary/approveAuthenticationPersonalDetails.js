 ...
 */
approveAuthenticationPersonalDetails = (function () {
    var form = $('#approveAuthenticationPersonalDetailsFormId');

    function _baseURL() {
        return 'approveAuthenticationPersonalDetails/';
    }
        ...
        $.ajax({
            ...
            , success: function (res) {
                ...
                if (res.responseStatus == 1) {
                    ...
                    $('#incorrectDate').val(formatAsDate(data.incorrectDate));
    ...
    function removeErrorMsg() {
        ...
        $('#appoinmentTime').on('keyup blur', function () {
            var value = $(this).val().trim();
            if (value != '') {
                $('#appoinmentTime').removeClass('error');
                $('#appoinmentTimeErrorMsg').text('');
            }
        });
    }
        ...
        $('.btnSave').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (res) {
                    var data = res.responseDTO;
                    if (res.responseStatus == 1) {
                        var url = _baseURL() + 'saveApproveAuthenticationPersonalDetails';
                        $.ajax({
                            url: url,
                            type: 'POST',
                            data: {
    ...
    function reject(){
        $('.btnSaves').on('click', function () {
            ...
            var url = _baseURL() + 'saveApproveAuthenticationPersonalDetailss';
            $.ajax({
                url: url,
                type: 'POST',
                data: {
...
$(document).ready(
    function () {
        approveAuthenticationPersonalDetails.disableTabs();
        approveAuthenticationPersonalDetails.getApplicationDetail();
        approveAuthenticationPersonalDetails.btnNext();
        approveAuthenticationPersonalDetails.btnPrevious();
        approveAuthenticationPersonalDetails.showHideRejectRemarks();
        approveAuthenticationPersonalDetails.removeErrorMsg();
        approveAuthenticationPersonalDetails.approveOrReject();
        approveAuthenticationPersonalDetails.reject();
        approveAuthenticationPersonalDetails.downloadFile();
        approveAuthenticationPersonalDetails.getAttachedFiles();
        approveAuthenticationPersonalDetails.downloadApplicationNo();
        approveAuthenticationPersonalDetails.downloadApplicationNoReject();
    });