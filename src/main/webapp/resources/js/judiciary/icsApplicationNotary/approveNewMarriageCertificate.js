 ...
 */
approveNewMarriageCertificate = (function () {
    var form = $('#approveNewMarriageCertificateFormId');

    function _baseURL() {
        return 'approveNewMarriageCertificate/';
    }
    ...
    function getApplicationDetail() {
        ...
        $.ajax({
            ...
            , success: function (res) {
                var val = res.serviceName;
                var data = res.responseDTO;
                var data1 = res.spouseDTO;
                var data2 = res.childDTO;
                var data3 = res.witness1DTO;
                var data4 = res.witness2DTO;
                ...
                if (res.responseStatus == 1) {
                    ...
                    populate(data4);
                    populate(data5);
                    $('#confirmChild').val(val);
                    if (val == 'Y') {
                        $('#confirmChildDiv').show();
                        populate(data2);
                        $('#childDob').val(formatAsDate(data2.childDoB));
                    }
                    else {
        ...
        $('.btnSave').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (res) {
                    var data = res.responseDTO;
                    if (res.responseStatus == 1) {
                        var url = _baseURL() + 'saveapproveNewMarriageCertificate';
                        $.ajax({
                            url: url,
                            type: 'POST',
                            data: {
    ...
    function reject() {
        $('.btnSaves').on('click', function () {
            ...
            rejectReasonId=vals;
            var url = _baseURL() + 'saveapproveNewMarriageCertificates?applicationNo='+applicationNo+'&remarks='+remarks+'&rejectedId='+rejectReasonId+'&mobileNo='+mobileNo;
            $.ajax({
                url: url,
                type: 'POST',
                success: function (res) {
...
$(document).ready(
    function () {
        approveNewMarriageCertificate.disableTabs();
        approveNewMarriageCertificate.getApplicationDetail();
        approveNewMarriageCertificate.downloadFile();
        approveNewMarriageCertificate.getAttachedFiles();
        approveNewMarriageCertificate.btnNext();
        approveNewMarriageCertificate.btnPrevious();
        approveNewMarriageCertificate.showHideRejectRemarks();
        approveNewMarriageCertificate.removeErrorMsg();
        approveNewMarriageCertificate.approveOrReject();
        approveNewMarriageCertificate.reject();
        approveNewMarriageCertificate.downloadApplicationNo();
        approveNewMarriageCertificate.downloadApplicationNoReject();
    });
