 ...
 */
@Service
public class ApproveAuthenticationPersonalDetailsService {
    @Autowired
    ...
    public ResponseMessage approveApplication(UserRolePrivilegeDto userBean, String applicationNo, String remarks, String rejectReasonId, Date appoinmentDate, String appoinmentTime, String mobileNo) throws ParseException {
        ...
        String courtName = this.commonService.getCourtName(courtId);
        ...
        if (rejectReasonId == null) {
            Boolean bool = Boolean.valueOf(CommonNotificationUtils.notifyOnVerification(responseMessage, appoinmentDateApproveStatus, appoinmentTimeApproveStatus, contactNums));
        } else {
            Boolean bool = Boolean.valueOf(CommonNotificationUtils.notifyOnRejection(responseMessage, contactNums, phoneNo));
        }
    ...
    public ResponseMessage approveApplications(UserRolePrivilegeDto userBean, String applicationNo, String remarks, Integer rejectReasonId, String rejectRemarks, String mobileNo) throws ParseException {
        ...
        try {
            ...
            if (rejectReasonId.intValue() != 0) {
                System.out.println("headCourt phone number --------------" + str + "------------------printed");
                Boolean var19 = Boolean.valueOf(CommonNotificationUtils.notifyOnRejection(responseMessage, contactNums, str));
            } else {
                Boolean bool = Boolean.valueOf(CommonNotificationUtils.notifyOnVerification(responseMessage, appoinmentDateApproveStatus, appoinmentTimeApproveStatus, contactNums));
            }
        } catch (Exception e) {
