...
import com.spring.mvc.judiciary.service.notaryService.ApproveChildTravelDocumentService;
 ...
 */
@Controller
@RequestMapping("/approveChildTravelDocument")
public class ApproveChildTravelDocumentController {
    @Autowired
    private ApproveChildTravelDocumentService approveChildTravelDocumentService;
    @Autowired
    ...
    public String index(ModelMap model) {
        if (model.size() != 0) {
            ...
            return "notary/approveChildTravelDocument";
        } else {
    ...
    @ResponseBody
    @RequestMapping(value = "/saveApproveChildTravelDocument", method = RequestMethod.POST)
    public ResponseMessage saveApproveChildTravelDocument(HttpServletRequest request,
        ...
        responseMessage = approveChildTravelDocumentService.approveApplication(userBean,
                applicationNo, remarks, rejectReasonId, appoinmentDate,appoinmentTime,mobileNo);
        return responseMessage;
    }

    @ResponseBody
    @RequestMapping(value = "/saveApproveChildTravelDocuments", method = RequestMethod.POST)
    public ResponseMessage saveApproveChildTravelDocuments(HttpServletRequest request, HttpServletResponse response,String applicationNo,String remarks, Integer rejectReasons,String mobileNo)
            throws Exception {
        String rejectRemarks= String.valueOf(rejectReasons);
        UserRolePrivilegeDto userBean = (UserRolePrivilegeDto) request.getSession().getAttribute("UserRolePrivilege");
        responseMessage = approveChildTravelDocumentService.approveApplications(userBean,
