...
import com.spring.mvc.judiciary.service.notaryService.ApproveAttestationDocumentService;
 ...
 */
@Controller
@RequestMapping("/approveAttestationDocument")
public class ApproveAttestationDocumentController {
    @Autowired
    private ApproveAttestationDocumentService approveAttestationDocumentService;
    @Autowired
    ...
    public String index(ModelMap model) {
        if (model.size() != 0) {
            ...
            List<DropdownDTO> reject_attest=(List<DropdownDTO>) this.commonService.getRejectAttest();
            model.addAttribute("reject_attest",(Object)reject_attest);
            model.addAttribute("religion_list", commonService.getAllReligion());
            model.addAttribute("court_list", commonService.getAllCourt());
            return "notary/approveAttestationDocument";
        } else {
    ...
    @ResponseBody
    @RequestMapping(value = "/saveApproveAttestationDocument", method = RequestMethod.POST)
    public ResponseMessage saveApproveAttestationDocument(HttpServletRequest request,
                                                          String applicationNo, String remarks,
                                                         Date appoinmentDate,String appoinmentTime, String mobileNo,String applicationYear)
            throws Exception {
        ...
        responseMessage = approveAttestationDocumentService.approveApplication(userBean,
                applicationNo, remarks, rejectReasonId, appoinmentDate,appoinmentTime,mobileNo,applicationYear);
        return responseMessage;
    }

    @ResponseBody
    @RequestMapping(value = "/saveApproveAttestationDocuments", method = RequestMethod.POST)
    public ResponseMessage saveApproveAttestationDocuments(HttpServletRequest request, HttpServletResponse response,String applicationNo,String remarks, Integer rejectReasons,String mobileNo)
            throws Exception {
        String rejectRemarks= String.valueOf(rejectReasons);
        UserRolePrivilegeDto userBean = (UserRolePrivilegeDto) request.getSession().getAttribute("UserRolePrivilege");
        responseMessage = approveAttestationDocumentService.approveApplications(userBean,
