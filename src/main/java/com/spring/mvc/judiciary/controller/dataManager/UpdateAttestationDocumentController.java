...
import com.spring.mvc.judiciary.service.dataManager.UpdateAttestationDocumentService;
 ...
 */
@Controller
@RequestMapping("/updateAttestationDocument")
public class UpdateAttestationDocumentController {
    @Autowired
    private UpdateAttestationDocumentService updateAttestationDocumentService;
    @Autowired
    ...
    public String index(ModelMap model) {
        if (model.size() != 0) {
            ...
            return "dataManager/updateAttestationDocument";
        } else {
    ...
    @ResponseBody
    @RequestMapping(value = "/updateAttestationDocument", method = RequestMethod.POST)
    public ResponseMessage updateAttestationDocument(HttpServletRequest request,HttpServletResponse response,
       ...
       // String applicationYear=applicationDetailDTO.getApplicationYear();
        UserRolePrivilegeDto userBean = (UserRolePrivilegeDto) request.getSession().getAttribute("UserRolePrivilege");
        responseMessage = updateAttestationDocumentService.updateApplication(userBean,
