...
import com.spring.mvc.judiciary.service.notaryService.ApproveClosingTransferAccountService;
 ...
 */
@Controller
@RequestMapping("/approveClosingTransferAccounts")
public class ApproveClosingTransferAccountsController {
    @Autowired
    private ApproveClosingTransferAccountService approveClosingTransferAccountService;
    @Autowired
    ...
    public String index(ModelMap model) {
        if (model.size() != 0) {
            ...
            return "notary/approveClosingTransferAccounts";
        } else {
    ...
    @ResponseBody
    @RequestMapping(value = "/saveApproveClosingTransferAccounts", method = RequestMethod.POST)
    public ResponseMessage saveApproveClosingTransferAccounts(HttpServletRequest request,
        ...
        responseMessage = approveClosingTransferAccountService.approveApplication(userBean,
                applicationNo, remarks, rejectReasonId,appoinmentDate,appoinmentTime,mobileNo);
        return responseMessage;
    }

    @ResponseBody
    @RequestMapping(value = "/saveApproveClosingTransferAccountss", method = RequestMethod.POST)
    public ResponseMessage saveApproveClosingTransferAccountss(HttpServletRequest request, HttpServletResponse response,String applicationNo,String remarks, Integer rejectReasons,String mobileNo)
            throws Exception {
        String rejectRemarks= String.valueOf(rejectReasons);
        UserRolePrivilegeDto userBean = (UserRolePrivilegeDto) request.getSession().getAttribute("UserRolePrivilege");
        responseMessage = approveClosingTransferAccountService.approveApplications(userBean,
