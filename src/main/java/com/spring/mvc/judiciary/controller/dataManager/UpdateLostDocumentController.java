...
import com.spring.mvc.judiciary.service.dataManager.UpdateLostDocumentService;
 ...
 */
@Controller
@RequestMapping("/updateLostDocument")
public class UpdateLostDocumentController {
    @Autowired
    private UpdateLostDocumentService updateLostDocumentService;
    @Autowired
    ...
    public String index(ModelMap model) {
        if (model.size() != 0) {
            ...
            return "dataManager/updateLostDocument";
        } else {
    ...
    @ResponseBody
    @RequestMapping(value = "/updateLostDocument", method = RequestMethod.POST)
    public ResponseMessage updateLostDocument(HttpServletRequest request, ApplicationDetailDTO applicationDetailDTO) throws Exception {
        ...
        UserRolePrivilegeDto userBean = (UserRolePrivilegeDto) request.getSession().getAttribute("UserRolePrivilege");
        responseMessage = updateLostDocumentService.updateApplication(userBean,
