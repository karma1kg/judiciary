...
import com.spring.mvc.judiciary.service.dataManager.UpdateChildAdoptionService;
 ...
 */
@Controller
@RequestMapping("/updateChildAdoption")
public class UpdateChildAdoptionController {
    @Autowired
    private UpdateChildAdoptionService updateChildAdoptionService;
    @Autowired
    ...
    public String index(ModelMap model) {
        if (model.size() != 0) {
            ...
            return "dataManager/updateChildAdoption";
        } else {
    ...
    @ResponseBody
    @RequestMapping(value = "/updateChildAdoption", method = RequestMethod.POST)
    public ResponseMessage updateChildAdoption(HttpServletRequest request,ApplicationDetailDTO applicationDetailDTO) throws Exception {
        ...
        UserRolePrivilegeDto userBean = (UserRolePrivilegeDto) request.getSession().getAttribute("UserRolePrivilege");
        responseMessage = updateChildAdoptionService.updateApplication(userBean,
